...
class SqlDB {
  ...
  _onCreate(Database db, int version) {
    db.execute(
        "CREATE TABLE contact (name STRING NOT NULL , number STRING NOT NULL PRIMARY KEY, describtion STRING NOT NULL , email STRING NOT NULL, image STRING NOT NULL)");
  }

  generateFirstContact() async {
    var data = await db;
    ...
    await data!.rawInsert(
        "INSERT INTO contact(name,number,describtion,email,image) VALUES ('${model.name}','${model.number}','${model.describtion}','${model.email}','${model.image}')");
  }

  generateLastContact() async {
    var data = await db;
    var model = ContactModel(
        name: 'Abdelrahman',
        describtion: 'Mobile developer',
        number: '01022299724',
        email: 'abdelrahmanhatim10@gmail.com',
